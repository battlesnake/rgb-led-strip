...
static void spi_transfer(uint8_t * txbuffer, uint8_t * rxbuffer)
{
	...
	transfer_buffer.tx_buf = (unsigned long) txbuffer;
	transfer_buffer.rx_buf = (unsigned long) rxbuffer;
	transfer_buffer.len = TRANSFER_LEN;
	transfer_buffer.delay_usecs = TRANSFER_DELAY;
	transfer_buffer.speed_hz = TRANSFER_SPEED;
	transfer_buffer.bits_per_word = TRANSFER_BPW;
